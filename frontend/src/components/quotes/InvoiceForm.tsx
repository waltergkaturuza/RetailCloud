...
export default function InvoiceForm({ invoice, onClose, onSuccess }: InvoiceFormProps) {
  ...
  const [formData, setFormData] = useState<Partial<Invoice>>({
    ...
    quotation: invoice?.quotation || undefined,
    line_items: invoice?.line_items || [{ item_description: '', quantity: 1, unit_price: 0, line_total: 0 }],
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: any) => {
      if (invoice?.id) {
        return api.patch(`/quotes/invoices/${invoice.id}/`, data)
      } else {
        return api.post('/quotes/invoices/', data)
      }
    },
    onSuccess: () => {
    ...
    onError: (error: any) => {
      ...
      if (errorData) {
        setErrors(errorData)
        toast.error(errorData.detail || 'Failed to save invoice')
      } else {
  ...
  return (
    <Modal isOpen={true} onClose={onClose} title={invoice?.id ? 'Edit Invoice' : 'New Invoice'}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '16px' }}>
              ...
              <div>
                ...
                <Select
                  ...
                  onChange={(e) => setFormData({ ...formData, quotation: e.target.value ? parseInt(e.target.value) : undefined })}
                  options={[
                    { value: '', label: 'None' },
                    ...(quotations?.filter((q: any) => q.status === 'accepted') || []).map((q: any) => ({
                      value: q.id,
                      label: `${q.quotation_number} - ${q.customer_detail?.name || ''}`,
                    })),
                  ]}
                />
              </div>
          ...
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
            ...
            <Button type="submit" variant="primary" disabled={saveMutation.isPending}>
              {saveMutation.isPending ? 'Saving...' : invoice?.id ? 'Update Invoice' : 'Create Invoice'}
            </Button>
          </div>
        </div>
      </form>
